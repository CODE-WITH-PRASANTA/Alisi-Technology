...
import NetworkPriceViewPrice from "./NetwokPriceViewPrice";
...
const NetworkSolution = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await API_URL.put(`/network-prices/${editingId}`, form);
        alert("Network Plan Updated ✅");
      } else {
        await API_URL.post("/network-prices", form);
        alert("Network Plan Created ✅");
      }
    ...
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
      ...
      >
        <h2 className="text-xl font-semibold">
          {editingId ? "Edit Network Plan" : "Add Network Plan"}
        </h2>
          ...
          {form.features.map((f, i) => (
            <span key={i} className="px-3 py-1 bg-[#1b2230] rounded-full text-sm text-white">
              ...
              <button type="button"
                ...
                className="ml-2 text-red-400">✕</button>
            </span>
          ))}
        </div>
