...
const NetworkPriceViewPrice = ({ refreshKey, onEdit }) => {
  ...
  const fetchPrices = async () => {
    try {
      setLoading(true);
      const res = await API_URL.get("/network-prices"); // ✅ FIXED
      setPrices(res.data || []);
    } catch (err) {
  ...
  const deletePrice = async (id) => {
    if (!window.confirm("Delete this Network plan?")) return;

    try {
      await API_URL.delete(`/network-prices/${id}`); // ✅ FIXED
      fetchPrices();
    } catch (err) {
  ...
  return (
    <div className="bg-[#0f141b] border border-slate-800 rounded-2xl flex flex-col h-[500px]">
      ...
      <div className="p-6 border-b border-slate-800">
        <h2 className="text-xl font-semibold">Network Price Management</h2>
      </div>
      ...
      <div className="flex-1 overflow-auto">
        {loading ? (
          ...
          <table className="w-full text-sm border-collapse">
            <thead className="sticky top-0 bg-[#0f141b]">
...
export default NetworkPriceViewPrice;