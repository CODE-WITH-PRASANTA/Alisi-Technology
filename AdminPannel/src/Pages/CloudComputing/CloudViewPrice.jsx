...
const CloudViewPrice = ({ refreshKey, onEdit }) => {
  ...
  const deletePrice = async (id) => {
    if (!window.confirm("Delete this Cloud plan?")) return;

    try {
      await API_URL.delete(`/cloud-prices/${id}`);
      fetchPrices();
    } catch (err) {
  ...
  return (
    <div className="bg-[#0f141b] border border-slate-800 rounded-2xl flex flex-col h-[500px]">
      ...
      <div className="p-6 border-b border-slate-800">
        <h2 className="text-xl font-semibold">Cloud Price Management</h2>
      </div>
        ...
        {loading ? (
          ...
          <div className="min-w-[1200px]">
            <table className="w-full text-sm border-collapse">
              ...
              <tbody>
                {prices.map((p) => (
                  <tr key={p._id} className="border-b border-slate-800 hover:bg-[#161c25] transition">
                    <td className="py-3 px-4 font-medium">{p.title}</td>
                    <td className="px-4">{p.category || "CLOUD"}</td>
                    <td className="px-4 text-blue-400 font-semibold">{p.price}</td>
                    ...
                    <td className="px-4 text-right space-x-3">
                      <button onClick={() => onEdit?.(p)} className="text-blue-400 hover:text-blue-500">
                        <FiEdit />
                      </button>
                      <button onClick={() => deletePrice(p._id)} className="text-red-400 hover:text-red-500">
                        <FiTrash2 />
                      </button>
                    </td>
                  </tr>
