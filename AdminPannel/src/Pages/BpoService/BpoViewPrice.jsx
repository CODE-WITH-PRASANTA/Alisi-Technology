...
const BpoViewPrice = ({ refreshKey, onEdit }) => {
  ...
  const fetchPrices = async () => {
    try {
      ...
      console.log(prices); // DEBUG (remove later)
  ...
  const deletePrice = async (id) => {
    ...
    try {
      await API_URL.delete(`/bpo-prices/${id}`);
      fetchPrices();
    } catch (err) {
  ...
  return (
    <div className="bg-[#0f141b] border border-slate-800 rounded-2xl flex flex-col h-[500px]">
      ...
      <div className="p-6 border-b border-slate-800">
        <h2 className="text-xl font-semibold">BPO Price Management</h2>
      </div>
