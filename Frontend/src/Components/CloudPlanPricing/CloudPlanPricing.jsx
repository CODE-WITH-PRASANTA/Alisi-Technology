...
const AllServicesPlanPriceing = () => {
  ...
  return (
    <section className="cloudd-section">
      {/* Header */}
      <div className="cloudd-header">
        <span className="cloudd-tag">[ BEST PRICING ]</span>
        <h2>
          Pricing Plans to Find the <br /> Right IT Support
        </h2>

        {/* Toggle */}
        <div className="cloudd-toggle">
          <span className={!yearly ? "active" : ""}>Monthly</span>

          <div
            className={`cloudd-switch ${yearly ? "yearly" : ""}`}
      ...
      {/* Pricing Cards */}
      <div className="cloudd-grid">
        {plans.map((plan, i) => (
          <div
            key={i}
            className={`cloudd-card ${plan.featured ? "featured" : ""}`}
          >
            <h3>{plan.name}</h3>
            <p className="cloudd-sub">{plan.desc}</p>
